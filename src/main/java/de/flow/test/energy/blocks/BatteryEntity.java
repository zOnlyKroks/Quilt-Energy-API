...
public class BatteryEntity extends BlockEntity implements NetworkBlock {
	...
	private Store<AtomicDouble> store = new ComparatorUpdateStore<>(
			this,
			new LockableInput<>(new LimitedInput<>(() -> new AtomicDouble(storedAmount), aDouble -> storedAmount -= aDouble.get(), unit, new AtomicDouble(600.0)), () -> noOutput || redstoneNoOutput),
